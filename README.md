...
> **Status: scaffold.** The calculators are being ported task-by-task from the source app.
> Track progress with `rmap list` (see `roadmap/tasks.toml`). Nothing below is wired yet.
...
| `DeltaCalc.DCAPlanner` | `calculate_dca_ladder/1` — defensive + aggressive DCA orchestration |
| `DeltaCalc.PositionCalculator` | `calculate_position/2` — full sizing pipeline, takes a plain `config` map |
